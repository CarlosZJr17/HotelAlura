		...
		btnEliminar.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (filaReservas >= 0) {
					...
					JOptionPane.showMessageDialog(null, "Hubo un errro");
				}
			}
		});
		...
		Optional.ofNullable(modelo.getValueAt(tbReservas.getSelectedRow(), tbReservas.getSelectedColumn()))
				.ifPresent(fila -> {
					...
					try {
						...
						dataE = LocalDate.parse(modelo.getValueAt(tbReservas.getSelectedColumn(), 1).toString(),
								dateFormat);
						dataS = LocalDate.parse(modelo.getValueAt(tbReservas.getSelectedColumn(), 2).toString(),
								dateFormat);
					...
					// Para asegurar el no cambiar los id
					if (tbReservas.getSelectedColumn() == 0) {
						JOptionPane.showMessageDialog(this, "No se pueden cambiar los ID");
					} else {
	...
	private void actualizarHuespedes() {
		...
		.ifPresentOrElse(filaHuespedes ->{
			...
			if(tbHuespedes.getSelectedColumn()== 0 || tbHuespedes.getSelectedColumn()==6) {
				...
				this.huespedesController.actualizarH(nombre, apellido, null, nacionalidad, telefono, id_reserva, id_reserva);
				JOptionPane.showMessageDialog(this, String.format("Registro modificado con exito"));
			}
		...
		}, ()-> JOptionPane.showMessageDialog(this, "Respete los campos"));
	}
